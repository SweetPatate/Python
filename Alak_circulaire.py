...
def put(board, n, player,a,removed):
    ...
    if player != 1:
        ...
        for i in range(a+1, n):  # captures a droite du pion
            ...
            if i == n-1:  # si on atteint le bord de droite
                for k in range(0, a, 1):  # captures a la gauche du pion
                    if board[k] == 0:  # si une case est vide a gauche de a pas de capture
                        break
                    elif board[k] == 1:  # si un autre pion du meme joueur est present
                        for j in range(0, k):  # capture des pions entre les deux
                            removed.append(j)
                            board[j] = 0  # comme les pions entre les deux ne sont pas 0 ils sont forcemment 2 "noir(s)"
        ...
        for i in range(a-1, -1, -1):  # captures a la gauche du pion
            ...
            if i == 0:  # si on atteint le bord gauche
                for k in range(n-1, a+1, -1):  # captures a droite du pion
                    if board[k] == 0:  # si une case est vide break
                        break
                    elif board[k] == 1:  # si un pion blanc est trouver
                        for j in range(n-1, k, -1):  # capture les pions entre les deux pions blancs
                            removed.append(j)
                            board[j] = 0  # comme les pions entre les deux ne sont pas 0 ils sont forcemment 2 "noir(s)"
        ...
        for i in range(a + 1, n):  # on répéte l'opération pour le joueur 2
            ...
            if i == n - 1:  # si on atteint le bord de droite
                for k in range(0, a, 1):  # captures a la gauche du pion
                    if board[k] == 0:  # si une case est vide a gauche de a pas de capture
                        break
                    elif board[k] == 2:  # si un autre pion du meme joueur est present
                        for j in range(0, k):  # capture des pions entre les deux
                            removed.append(j)
                            board[j] = 0  # comme les pions entre les deux ne sont pas 0 ils sont forcemment 2 "noir(s)"
                        for j in range(n - 1, a, -1):  # capture des pion entre le bord et le pion
        ...
        for i in range(a - 1, -1, -1):
            ...
            if board[i] == 0:
                ...
                for j in range(i + 1, a):
                    ...
                    board[j] = 0
                break
                                                                                                                         #déplace le break pour enpecher d'éfaccer sur les cotés
            if i == 0:  # si on atteint le bord gauche
                for k in range(n-1, a + 1, -1):  # captures a droite du pion
                    if board[k] == 0:  # si une case est vide break
                        break
                    elif board[k] == 2:  # si un pion blanc est trouver
                        for j in range(n-1, k, -1):  # capture les pions entre les deux pions blancs
                            removed.append(j)
                            board[j] = 0  # comme les pions entre les deux ne sont pas 0 ils sont forcemment 2 "noir(s)"
                        for j in range(0, a):  # capture des pion entre le bord et le pion
...
print("! ATTENTION ! ce tableau est circulaire")
